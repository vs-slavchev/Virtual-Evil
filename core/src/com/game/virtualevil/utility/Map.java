...
public class Map {
    ...
    public static final int NO_COLLISION_LAST_TILE_ID = 80;
    public static final int HALF_COLLISION_LAST_TILE_ID = 130;
    public static final int FULL_COLLISION_LAST_TILE_ID = 255;
    // map
    ...
    public Map(PlayGameState playGameState, String mapName) {
        ...
        renderDistanceInIndices = (Gdx.graphics.getWidth() / 2) / (2 * tileSize) + 2;
    }
    ...
    private boolean collidesWithTile(final Rectangle characterCollisionRectangle,
		 ...
		 * 80-130 -> 50%
		 * 130-255 -> 100% */
        if (getTileID(mapXindex, mapYindex) < NO_COLLISION_LAST_TILE_ID) {
            return false;
        } else if (getTileID(mapXindex, mapYindex) < HALF_COLLISION_LAST_TILE_ID) {
        ...
        } else if (getTileID(mapXindex, mapYindex) < FULL_COLLISION_LAST_TILE_ID) {
