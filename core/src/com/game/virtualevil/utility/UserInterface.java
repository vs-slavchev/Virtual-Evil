...
public class UserInterface {
	...
	public UserInterface (TextureManager tm){
		...
		for (int i = 0; i < abilityNames.length; i++){
			TextureRegion image = new TextureRegion(
					tm.getImage("abilitiesTileSet"),0 ,i * 32 ,32, 32);
	...
	private void drawAbilitiesInterface(SpriteBatch batch, PlayerCharacter player) {
		...
		for(int i = 0; i < player.getAbilities().size();i++ ){
			TextureRegion icon = getAbilitiesMap().get(
					player.getAbilities().get(i).getAbilityName());
	...
	private void drawDigitsHealth(SpriteBatch batch, PlayGameState playState) {
		...
		lcdFont.draw(batch, Integer.toString(playState.getEntityManager().getPlayer().getCurrentHealth()),
				width - 365, height - 45);
	}

	private void drawWeaponsInterface(SpriteBatch batch, PlayGameState playState) {
		batch.draw(weaponsInterfacePanel, width - weaponsInterfacePanel.getRegionWidth()- 210,
				height - weaponsInterfacePanel.getRegionHeight() - 980, 414, 192);
	...
	private void cropInterfaceRegions(TextureManager tm) {
		healthAndEnergyInterface = new TextureRegion(
				tm.getImage("HPETileSet"), 0, 0, 384, 196);
		minimapInterface = new TextureRegion(
				tm.getImage("HPETileSet"), 0, 518, 120, 120);
		abilitiesInterface = new TextureRegion(
				tm.getImage("HPETileSet"), 0, 657, 298, 112);
		weaponsInterfacePanel = new TextureRegion(
				tm.getImage("HPETileSet"), 0, 411, 207, 96);
		...
		energyBar = new TextureRegion(tm.getImage("HPETileSet"), 0, 329, 294, 78);
