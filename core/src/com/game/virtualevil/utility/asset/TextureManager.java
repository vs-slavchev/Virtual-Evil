...
public class TextureManager {
    ...
    /**
     * Magical code to fix the obscure tilemap bleeding bug in libgdx;
    ...
    private void cropInterfaceRegions() {
        ...
        for (int i = 0; i < abilityNames.length; i++) {
            TextureRegion image = new TextureRegion(texRegions.get("abilitiesTileSet"),
                    0, i * 32, 32, 32);
            texRegions.put(abilityNames[i], image);
        }

        for (int j = 0; j < weaponNames.length; j++) {
            TextureRegion sprite = new TextureRegion(texRegions.get("weaponsTileSet"),
                    0, j * 64, 64, 64);
